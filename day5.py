...
while position < len(output):
  opcode = "{:05d}".format(output[position])
  modes = opcode[:3]
  opcode = opcode[-2:]
  
  if opcode == '01':
    opcode1(*getParameterValues(modes, position + 1, position + 2, position + 3))
    position += 4
  elif opcode == '02':
    opcode2(*getParameterValues(modes, position + 1, position + 2, position + 3))
    position += 4
  elif opcode == '03':
    opcode3(*getParameterValues(modes, position + 1))
    position += 2
  elif opcode == '04':
    opcode4(*getParameterValues(modes, position + 1))
    position += 2
  elif opcode == '99':
    print(output)
    position = len(output)
