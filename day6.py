...
class Planet:
  ...
  def __str__(self):
    return f'{"" if self.orbiting is None else self.orbiting.name}){self.name}'
...
class OrbitMap:
  ...
  def __str__(self):
    return '\n'.join([f'{planet}' for planet in self.planets])

  def findOrCreatePlanet(self, name, orbiting = None):
  ...
  def addOrbitToMap(self, parentBodyName, satelliteName):
    parentBody = self.findOrCreatePlanet(parentBodyName)
